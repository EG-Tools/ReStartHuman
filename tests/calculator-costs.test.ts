...
test('보험료는 납입기간이 끝나면 이후 연차부터 지출에서 제외한다', () => {
  const projection = calculateCashProjection(
    {
      ...defaultFormData,
      inflationEnabled: false,
...
test('피부양자 배당이 정확히 2,000만원이면 high가 아니라 review로 본다', () => {
  const annualGross = policyConfig.healthInsurance.employeeAdditionalIncomeThresholdAnnual
  ...
  const assessment = getDependentHealthInsuranceAssessment({
    ...
    totalDividendAnnualGross: annualGross,
    age: 50,
    pensionMonthly: 0,
  })
  const premium = estimateHealthInsurance(
...
test('previous year declared business income increases the insurance estimate', () => {
  const withoutDeclaredIncome = estimateHealthInsurance(
    {
      ...defaultFormData,
      healthInsuranceType: 'regional',
  ...
  const withDeclaredIncome = estimateHealthInsurance(
    {
      ...defaultFormData,
      healthInsuranceType: 'regional',
      selectedIncomeCategories: ['business'],
